...
def simulate(grid,start,steps):
	flash_count = 0 		# Count of flash events
	...
	for step in range(start,start+steps):
		...
		# Increase count by size of the flashed set
		flash_count += len(flash_set)

		# Status reports!
		if step <= 10:
			ic(step,grid,len(flash_set),flash_count)
		elif step % 10 == 0:
			ic(step,grid,len(flash_set),flash_count)

		# Reset flashed set and queue
		flash_set.clear() 
		flash_queue.clear()
...
def test():
	...
	assert simulate(grid,0,100) == 1656

def main():
	ic.disable()
	grid = [[int(n) for n in line] for line in aocd.data.splitlines()]
	aocd.submit(simulate(grid,0,100), part='a')
